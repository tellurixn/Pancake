...
public:
	...
	bool isEmpty() { return head == 0; }  // Проверка, пуст ли список

	Element* getValue(Element* p) { return p->field; } // Получение значения узла списка

	void setValue(Element* p, Element const& val) { p->field = val; } // Установка значения узла списка

	Element* getFirst() { return head; } // Получение корневого узла списка
	...
	Element* Add(T& t, Element* node = 0) {
		...
		count++;
		if (node == 0) // Добавление нового корня
		{
			if (head == 0) {
				elem->ptr = 0;
				head = elem;
